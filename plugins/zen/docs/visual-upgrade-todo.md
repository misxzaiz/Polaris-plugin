...
| 优先级 | 内容 | 状态 |
|--------|------|------|
| P0 | 小僧 Canvas 动画 | ✅ 已完成（`src/MonkCanvas.tsx`） |
| P1 | 木鱼 CSS 绘制 + 敲击动画 | ✅ 已完成（`src/zenStyles.ts` + `.muyu-btn`） |
| P2 | 抽签动画 | ✅ 已完成（签筒摇晃 + 签条滑出 + 运签光晕） |
| P3 | 答案之书翻页动画 | ✅ 已完成（CSS 3D 翻页 + 内容渐入） |
| P4 | 日记时间线视觉增强 | ✅ 已完成（统计卡片 + 时间线圆点） |

**实现要点**：
- 动画样式集中在 `src/zenStyles.ts`（`ensureZenStyles()` 一次性注入）
- 小僧用 `src/MonkCanvas.tsx`（Canvas 2D，`requestAnimationFrame` 驱动，仅面板激活时运行）
- 所有动画支持 `prefers-reduced-motion: reduce`（减弱系统动画偏好）
